...
function generateId(): string {
  return `q_${Date.now()}_${Math.random()
    .toString(36)
    .slice(2, 9)}`;
}
...
export function useInterview() {
  ...
  const resumeInterviewQuestionsRef =
    useRef<ResumeInterviewQuestion[]>(
      []
    );

  const resumeInterviewIndexRef =
    useRef(0);

  const isFinishingRef =
    useRef(false);

  const isAdvancingRef =
    useRef(false);

  const interviewIdRef =
    ...
    useCallback(
      async (
        ...
        if (
          ...
          const nextResumeQuestion =
            resumeQuestions[
              resumeInterviewIndexRef
                .current
            ];

          if (
            !nextResumeQuestion?.question?.trim()
          ) {
            throw new Error(
              "The resume-based interview does not contain a valid next question."
            );
          }

          resumeInterviewIndexRef.current +=
            1;

          const question: InterviewQuestion =
            {
              id:
                nextResumeQuestion.id ??
                generateId(),
        ...
        /*
         * Normal AI-generated question
         */
        const data =
          await postInterviewApi<{
            question?: unknown;
          }>({
            action: "question",
        ...
        if (
          ...
          throw new Error(
            "The interview API did not return a valid question."
          );
        }
    ...
    useCallback(
      async (
        ...
        try {
          ...
          console.error(
            "Initial interview question failed:",
            message
          );
    ...
    useCallback(
      async (
        ...
        try {
          ...
          const evaluation: AnswerEvaluation =
            {
              ...
              improvementSuggestions:
                Array.isArray(
                  evaluationData.improvementSuggestions
                )
              ...
              modelAnswer:
                typeof evaluationData.modelAnswer ===
                "string"
                  ? evaluationData.modelAnswer
                  : undefined,

              coachingMessage:
                typeof evaluationData.coachingMessage ===
                "string"
                  ? evaluationData.coachingMessage
                  : undefined,

              followUp:
                typeof evaluationData.followUp ===
                "boolean"
           ...
           *
           * We intentionally don't await this here.
           * The final save will happen when the
           * interview finishes.
           */
          setTimeout(() => {
            void saveAnswerImmediately();
          }, 0);
          ...
          console.error(
            "Interview answer request failed:",
            message
          );
  ...
  const generateFeedback =
    useCallback(
      async (): Promise<InterviewFeedback | null> => {
        const currentState =
          stateRef.current;
        ...
        try {
          const feedback =
            await postInterviewApi<InterviewFeedback>(
              {
                action: "feedback",
   ...
   * aegis_session
   *
   * because credentials: "include" is used
   * by postInterviewApi().
   */
  const saveInterview =
    useCallback(
      async (
        feedbackOverride?: InterviewFeedback | null
      ): Promise<boolean> => {
        const currentState =
          stateRef.current;
        ...
        if (
          ...
          console.error(
            "[saveInterview] Aborted: missing config or feedback",
            {
              hasConfig:
                !!currentState.config,
        ...
        try {
          ...
          console.log(
            "[saveInterview] Sending MongoDB save request:",
            {
              role:
                savePayload.role,

              interviewType:
                savePayload.interviewType,

              difficulty:
                savePayload.difficulty,
              ...
              durationSeconds:
                savePayload.durationSeconds,

              startedAt:
                savePayload.startedAt,

              existingInterviewId:
                savePayload.interviewId,
            }
          );
          ...
          console.log(
            "[saveInterview] Save API response:",
            result
          );
          ...
          console.error(
            "[saveInterview] Save request failed:",
            {
              message,
              error:
      ...
      async (): Promise<{
        ...
        if (
          ...
          return {
            ...
            feedback: null,
            error:
              "Interview is already finishing.",
          };
        }
   ...
   *
   *
   * JWT cookie is automatically
   * sent to /api/interview.
   */
    ...
    useCallback(
      async (): Promise<boolean> => {
        ...
        try {
          ...
          console.log(
            "[saveAnswerImmediately] Saving interview progress..."
          );
          ...
          console.error(
            "[saveAnswerImmediately] Save failed:",
            message
          );
